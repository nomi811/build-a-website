...
Did it work?  It should have.  But it's not very pretty and HTML pages can be _very long_, so this approach is not going to scale very well.  Ideally, we want the HTML in a separate file that we can just load and return.  Again, this is so fundamental it's already built into Sinatra.
